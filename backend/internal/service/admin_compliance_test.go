...
func TestAdminComplianceStatusRequiresAckWhenMissing(t *testing.T) {
	...
	// [s2a fork] The admin compliance gate is disabled for this enterprise-internal
	// deployment (GetAdminComplianceStatus defaults Required=false), so a missing
	// acknowledgement no longer forces the gate. Metadata fields stay populated.
...
func TestAdminComplianceStatusRequiresAckOnOldVersion(t *testing.T) {
	...
	status, err := svc.GetAdminComplianceStatus(context.Background(), 1)
	require.NoError(t, err)
	// [s2a fork] Gate disabled: an outdated acknowledgement version no longer
	// forces Required. The stale ack is still treated as non-current (Acknowledgement nil).
	require.False(t, status.Required)
...
func TestAdminComplianceStatusIsPerAdminUser(t *testing.T) {
	...
	require.NoError(t, err)
	// [s2a fork] Gate disabled: a second admin without an acknowledgement is also
	// not required to acknowledge (upstream would expect Required=true here).
