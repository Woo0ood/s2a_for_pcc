...
func (s *SettingService) GetAdminComplianceStatus(ctx context.Context, adminUserID int64) (*AdminComplianceStatus, error) {
	status := &AdminComplianceStatus{
		// [s2a fork] Enterprise-internal deployment: the admin compliance
		// acknowledgement gate is disabled by default. Compliance for this
		// private instance is owned by the operator out-of-band, so we do not
		// force the upstream acknowledgement dialog. Keeping this as a one-line
		// fork divergence (vs. removing the AdminComplianceGuard wiring in
		// routes/admin.go) confines the change to this low-churn file and
		// minimizes future merge conflicts. AcceptAdminCompliance still works
		// if an operator chooses to record an acknowledgement explicitly.
		Required:       false,
