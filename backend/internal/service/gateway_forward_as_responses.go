...
func (s *GatewayService) handleResponsesBufferedStreamingResponse(
	...
	if respBytes, err := json.Marshal(responsesResp); err == nil {
		...
		// Payload audit: tee the full Responses JSON body (post tool-restore) for structured fidelity.
	...
	processEvent := func(event *apicompat.AnthropicStreamEvent) bool {
		...
		for _, evt := range events {
			...
			for _, restored := range payloads {
				...
				// Payload audit: tee each restored Responses SSE event exactly as written to the client.
				if auditColl := GetPayloadAuditCollector(c); auditColl != nil {
					auditColl.AppendRawEvent([]byte(fmt.Sprintf("event: %s\ndata: %s\n\n", eventType, restored)))
				}
