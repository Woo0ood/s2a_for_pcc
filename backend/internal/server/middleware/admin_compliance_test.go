...
// deployment (SettingService.GetAdminComplianceStatus defaults Required=false), so
// the guard no longer blocks admin routes when no acknowledgement is present.
// Upstream asserts http.StatusLocked + ADMIN_COMPLIANCE_ACK_REQUIRED here.
func TestAdminComplianceGuardAllowsAdminRouteWhenGateDisabled(t *testing.T) {
