...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	// 2. Re-check billing
	billingErr := h.billingCacheService.CheckBillingEligibility(requestCtx, apiKey.User, apiKey, apiKey.Group, subscription, service.QuotaPlatform(requestCtx, apiKey))
	{
		platform := ""
		if apiKey.Group != nil {
		...
		body, fallbackEngaged, billingErr = applyUserRateLimitFallback(requestCtx, body, reqModel, apiKey.UserID, platform, billingErr, h.settingService)
		if fallbackEngaged {
