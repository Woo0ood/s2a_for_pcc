...
// The PayloadAuditLogFunc type is an adapter to allow the use of ordinary
// function as PayloadAuditLog mutator.
type PayloadAuditLogFunc func(context.Context, *ent.PayloadAuditLogMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f PayloadAuditLogFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.PayloadAuditLogMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PayloadAuditLogMutation", m)
}
