    ...
    <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
      <thead class="table-header bg-gray-50 dark:bg-dark-800">
        <tr ref="headerRowRef">
          ...
          >
            <div :class="['relative flex items-center space-x-1', getHeaderContentAlignmentClass(column)]">
              <slot
...
// immediate:false + watch — the table (and header row) is v-else'd away in the mobile
// card layout, so auto-init on mount would hand Sortable a null element and throw.
const columnDraggable = useDraggable(headerRowRef, renderColumns, {
  immediate: false,
...
watch(
  ...
  (el, _prev, onCleanup) => {
    if (!el) return
    columnDraggable.start()
    onCleanup(() => columnDraggable.destroy())
  },
  { flush: 'post' }
)
